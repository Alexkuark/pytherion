...
def convert_tro(fle_tro_fnme, cavename = None):	
	...
	cavename = cavename.replace(u'̀', u'')
	# create .th file name 
...
#		settings[4:4] = u' '
	...
	if u'Topof' in settings:
		...
		#else:
		#	file.write(u'\t\tunits counter meters \n')
...
def convertdata(settings, data, stations):
	...
	#old version : use LRUD, Topof and prof (depth) from previous line when not defined
	"""
	...
	if u'Prof' in settings :
		...
		for i, elems in enumerate(data):
			#prof = float(elems[4])
			#if u'Dir' in dirs[1] :
				#data[i][4] = str(round((-prof0 + prof),2))
			#else :
				#data[i][4] = str(round((+prof0 - prof),2))
			#prof0 = prof
			prof = float(elems[4])
			if 	elems[0] != elems[1] :
				data[i][4] = str(prof0) + ' ' + str(prof)
			prof0 = prof
	...
	for i, elems in enumerate(data):
		for k in [0,1,2]:
			...
			# remove the '*', and replace them with the right data !
				if k == 1 : elems[k] = u'-'
				elif i == 0 and dataold[len(dataold)-1][k+1] != u'-' : elems[0] = dataold[len(dataold)-1][1]
				elif i == 0 and dataold[len(dataold)-1][k+1] == u'-' : elems[0] = dataold[len(dataold)-1][0]
				elif data[i-1][k+1] == u'-': elems[0] = data[i-1][k]
...
#			if elems[k] == u'*': elems[k] = u'0'
			if elems[k] == u'*':
				# Check that LRUD != '*'; If first line, change them to 0
				if i == 0 and len(dataold) == 0 : elems[k] = u'0.0'
				#Else, change them to LRUD from the previous line
				elif i == 0 and len(dataold) > 0 : elems[k] = dataold[len(dataold)-1][k]
				else: elems[k] = data[i-1][k] 
			elif i == 0 and len(dataold) > 0 and dataold[len(dataold)-1][0] == dataold[len(dataold)-1][1] and dataold[len(dataold)-1][0] == data[0][0]:
				data[0][k] = '['+dataold[len(dataold)-1][k]+' '+data[0][k]+']'

	i=0
	for elems in data:
		if 	elems[0] == elems[1] :
		# remove duplicates of entrance station used by visualtopo for LRUD purpose
			if i+1 < len(data) and elems[0] == data[i+1][0] and data[i+1][1] != u'-':
				k = dictl[settings[0]]-4
				LRUD = elems[k:k+4]
				for j in range(4) :
					data[i+1][k+j] = '['+LRUD[j]+' '+data[i+1][k+j]+']' 
			del data[i]
		elif float(elems[2]) == 0.0 :
